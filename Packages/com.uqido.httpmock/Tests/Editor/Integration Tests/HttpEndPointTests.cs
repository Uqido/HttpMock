...
////
        ...
        private static readonly (int wait, int epsilon)[] Delayed_stub_shouldnt_block_undelayed_stub_input =
        {
            (407, 50),
            (1015, 50),
            (1691, 50),
        ...
        [UnityTest]
        public IEnumerator Delayed_stub_shouldnt_block_undelayed_stub(
            [ValueSource(nameof(Delayed_stub_shouldnt_block_undelayed_stub_input))]
            (int wait, int epsilon) inputs) => UniTask.ToCoroutine(async () =>
        {
            var (wait, epsilon) = inputs;
            ...
            using (var wcDelayed = new WebClient())
            {
                ...
                taskDelayed = MeasureRequestTime(swDelayed, $"{_hostUrl}/firstEndp");
                
                wcUndelayed.DownloadString($"{_hostUrl}/secondEndp");

                taskUndelayed = MeasureRequestTime(swUndelayed, $"{_hostUrl}/secondEndp");
            }

            Assert.AreEqual("Delayed response (stub 1)", await taskDelayed);
            Assert.AreEqual("Undelayed response (stub 2)", await taskUndelayed);
        ...
        private static async UniTask<string> MeasureRequestTime(Stopwatch sw, string requestUrl)
        {
            // ReSharper disable once ConvertToUsingDeclaration
            using (var wc = new WebClient())
            {
                sw.Restart();
                var ans = await wc.DownloadStringTaskAsync(requestUrl);
                sw.Stop();
                return ans;
            }
        }
